...
**GET** **https://<endpoint-url>/info/configuration**
    ...
    - **max_batch_records**: the maximum number of records that can be
      uploaded to a collection as part of a batched upload.

    - **max_batch_bytes**: the maximum combined size in bytes of the
...
**POST** **https://<endpoint-url>/storage/<collection>**
    ...
    *X-Weave-Batch-Records** and *X-Weave-Batch-Bytes* headers to signal
    the expected batch size, so that oversize batches can be rejected before
    the items are uploaded.
...
atomic unit.  This is achieve by using the **batch** and **commit** parameters
